...
-- CAST converte em qualquer SGBD
-- CONVERT só converte no SQLserver

-- select 'A' + CAST (1 AS VARCHAR(1))

SELECT 'A' + '1' --- CONCACATENA POR SER DUAS STRNGS
SELECT 'A' + 1  -- NÃO CONVERT POR QUE E STRING E UM NUMERAL 

SELECT 'A' + CAST(1 AS VARCHAR(1))

SELECT PRODUTO 
, VENDA_LIQUIDA / QUANTIDADE AS VALOR_UNITARIO 
FROM VENDAS_ANALITICAS
...
SELECT ENTIDADE
, DATA_CADASTRO
, CAST (DATA_CADASTRO AS DATE) AS 'CAST'
, CONVERT(DATE, DATA_CADASTRO) AS 'CONVERT'
, CONVERT (VARCHAR, DATA_CADASTRO, 103) AS DATA_3 -- CUIDADO POR QUE ELE CONVERTE PARA VARCHAR E NÃO PARA DATE 
FROM ENTIDADES
ORDER BY DATA_3

-- TRY_CAST - tente coverter 
select try_cast( 'abc' as int)

select ENTIDADE, TRY_CAST (nome as int)
from ENTIDADES
---------------------------------------------------------
---DESAFIO------
---------------------------------------------------------

/* RECUPERE AS INFORMAÇOES:
1. VENDEDOR 
2. MÉDIA DE VENDAS
3. TIPO DE VENDA

DA TABELA VENDAS_ANALITICAS, APENAS DO VENDEDOR DE CODIGO 16, 
DO ANO DE 2020 PARA FRENTE SABENDO QUE 

1. A MEDIA DE VENDA E UMA MEDIA ARITIMETICA DA COLUNA VENDA_LIQUIDA
2 O TIPO DE VENDA SERÁ TRATADO PELO CAMPO OPERAÇÃO_FISCAL E QUANTIDADE DE FORMA QUE 
-> SEMPRE QUE A QUANTIDADE FOR MENOR DO QUE ZERO DEVERÁ SER EXIBIDO DEVOLUÇÃO 
-> SEMPRE QUE A OPERÇÃO FISCAL FOR OS CODIGOS 12 OU 14 DEVERÁ SER EXIBIDO VENDA INTERNA 
-> PARA TODOS OS DEMAIS CASOS EXIBA VENDA externa

* EXIBA A MEDIA DE VENDA COM SOMENTE 2 CASAS DECIMAIS
*/
select * from VENDAS_ANALITICAS

SELECT VENDEDOR AS VENDEDOR 
, CAST(ROUND (AVG(VENDA_LIQUIDA ),2)AS numeric (15,2)) AS MEDIA_VENDA_LIQUIDA
, case when QUANTIDADE < 0
	then 'Devolulção'
	when OPERACAO_FISCAL IN(12,14)
	then 'Venda interna'
	else 'Venda externa'
end as Tipos_de_venda 
FROM VENDAS_ANALITICAS
where VENDEDOR = 16 AND MOVIMENTO >= '01/01/2020'
group by VENDEDOR
,case when QUANTIDADE < 0
	then 'Devolulção'
	when OPERACAO_FISCAL IN(12,14)
